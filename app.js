...
const state = { workbook:null, sheetName:'', employees:[], filtered:[], current:null, printMode:'one' };
...
function openReport(e){state.current=e;state.printMode='one';$('reportPreview').innerHTML=reportHTML(e);$('reportModal').classList.remove('hidden');document.body.style.overflow='hidden';scaleModal()}
...
$('printAll').onclick=()=>{state.printMode='all';$('reportPreview').innerHTML=state.employees.map(reportHTML).join('');$('reportModal').classList.remove('hidden');document.body.style.overflow='hidden';scaleModal();setTimeout(()=>window.print(),150)};
